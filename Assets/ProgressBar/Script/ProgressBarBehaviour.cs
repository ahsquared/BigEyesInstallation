...
namespace ProgressBar
{
    ...
    public class ProgressBarBehaviour : MonoBehaviour, IIncrementable, IDecrementable
    {
        ...
        public float[] BarBeat
        {
            
            set
            {
                SetFillerSizeAsBars(value);
            }
        }
        ...
        void Update()
        {
            ...
            if (TransitoryValue != m_Value.AsFloat)
            {
                ...
                {
                    TransitoryValue = 0; //ProgressSpeed * Time.deltaTime;
                    if (TransitoryValue < m_Value.AsFloat)
                        TransitoryValue = m_Value.AsFloat;
                }
                ...
                SetFillerSize(TransitoryValue >= m_FillerInfo.MaxWidth ? 0 : TransitoryValue);
                //Debug.Log("filler width: " + TransitoryValue);
...
//                m_AttachedText.text = Mathf.Round(Width / FillerInfo.MaxWidth * 100).ToString() + " %";
        ...
        /// <summary>
        /// This method is used to set the Filler's width as fraction of 4 bar phrase
        /// </summary>
        /// <param name="barBeat">this method needs an array of the [bar, beat] as parameter</param>
        public void SetFillerSizeAsBars(float[] barBeat)
        {
            float percent = 100 * (((barBeat[0] - 1) * 4 + barBeat[1] - 1) / 16f);

            if ((int)barBeat[0] == 1 && (int)barBeat[1] == 1)
            {
                percent = 100;
            }
            ...
            if (m_AttachedText)
                m_AttachedText.text = barBeat[0] + " : " + barBeat[1];
